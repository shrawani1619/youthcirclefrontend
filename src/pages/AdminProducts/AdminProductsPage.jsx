...
const NUMBER_SIZES = Array.from({ length: 11 }, (_, i) => String(30 + i));
...
const AdminProductsPage = ({ mode = "list" }) => {
  ...
  const addPath = "/admin/products/add";
  const listPath = "/admin/products/list";
  ...
  const loadAdminData = async () => {
    ...
    try {
      const [productsData, vendorsData] = await Promise.all([
        getProducts({ limit: 50, sortBy: "createdAt", order: "desc" }),
        getAllVendors(token),
      ]);

      const nextVendors = vendorsData.vendors || [];
      setProducts(productsData.products || []);
      setVendors(nextVendors);
      setForm((current) => ({
        ...current,
        vendorId: current.vendorId || nextVendors.find((vendor) => vendor.isApproved)?._id || "",
      }));
      setError("");
    } catch (requestError) {
      setProducts([]);
      setVendors([]);
      setError(requestError.response?.data?.message || "Could not load admin product data.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const resetForm = () => {
    setForm({
      ...emptyForm,
      vendorId: approvedVendors[0]?._id || "",
    });
    setEditingId("");
  };
  ...
  const populateForm = (product) => {
    ...
    setForm({
      vendorId: product.vendorId?._id || approvedVendors[0]?._id || "",
      name: product.name || "",
  ...
  useEffect(() => {
    ...
    setForm((current) => ({
      ...emptyForm,
      vendorId: current.vendorId || approvedVendors[0]?._id || "",
    }));
  }, [approvedVendors, editProductId, isAddView, products]);
  ...
  return (
    <DashboardLayout
      ...
      subtitle={
        isAddView
          ? "Create a new product with a clean admin form layout."
          : "Manage and review products across the marketplace."
      }
      links={adminPanelLinks}
      panelLabel="Admin Panel"
      topBarSearchPlaceholder="Search products, vendors, or SKU"
        ...
        <section className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-soft lg:p-8">
            ...
            <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
              <div>
                ...
                <p className="mt-2 text-sm text-slate-500">
                  Manage {filteredProducts.length} products across approved vendors.
                </p>
              </div>
              ...
              <div className="flex w-full flex-col gap-3 xl:max-w-2xl xl:flex-row">
                <label className="flex flex-1 items-center gap-3 rounded-2xl border border-slate-200 px-4 py-3">
                  ...
                  <input
                    ...
                    placeholder="Search products, vendor, or category"
          ...
          {loading ? (
            ...
            <div className="mt-6 overflow-hidden rounded-[28px] border border-slate-200">
              <div className="hidden grid-cols-[2.2fr_1.2fr_1.2fr_0.8fr_0.9fr_1fr] gap-4 border-b border-slate-200 bg-slate-50 px-5 py-4 text-xs font-semibold uppercase tracking-[0.16em] text-slate-500 lg:grid">
                <span>Product</span>
                <span>Vendor</span>
                <span>Category</span>
                <span>Stock</span>
              ...
              <div className="divide-y divide-slate-200">
                {filteredProducts.map((product) => (
                  <article
                    key={product._id}
                    className="grid gap-4 px-5 py-5 lg:grid-cols-[2.2fr_1.2fr_1.2fr_0.8fr_0.9fr_1fr] lg:items-center"
                  >
                    ...
                    <p className="text-sm text-slate-600">{product.vendorId?.businessName || "Unknown vendor"}</p>
              ...
              <form onSubmit={handleSubmit} className="mt-6 max-w-3xl space-y-5">
                {!approvedVendors.length ? (
                  <p className="text-sm font-medium text-rose-600">
                    No approved vendor is available yet.
                  </p>
                ) : null}
